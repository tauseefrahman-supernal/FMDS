...
/** Grounded answer about one specific KPI: live rag/actual/target + Mark's own note. */
function replyForKpi(dept, dc, kpi) {
...
function summarizeReds(deptCtx, dc) {
  ...
  const lines = redKpis.map((k) => `• ${k.name} — ${k.explanation.why} (owner: ${k.owner})`);
  let out = `You have ${reds.length} red${reds.length === 1 ? '' : 's'} right now in ${deptName}:\n${lines.join('\n')}`;
  if (dc && dc.redStory) out += `\n\n${dc.redStory}`;
  return out;
}
...
function groundedReply(deptId, intent, ctx, deptCtx) {
  ...
  if (mentioned) return replyForKpi(ctx.dept, dc, mentioned);
