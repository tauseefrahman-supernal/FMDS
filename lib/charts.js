...
export function svgRecoveryTrend(points, opts = {}) {
  ...
  // Dots coloured per point via ragStatus (green/amber/red/nodata).
  const dots = points
    .map((v, i) => {
      if (typeof v !== 'number' || Number.isNaN(v)) return '';
      const status = ragStatus(v, target, 'higher_better');
...
export function svgPareto(rows, opts = {}) {
  ...
  const numeric = rows
    .map(r => r.value)
    .filter(v => typeof v === 'number' && !Number.isNaN(v));
  if (numeric.length === 0) return _noDataSvg(width, height || 90);
  ...
  const trackW  = width - PAD_L - PAD_R;
  const totalH  = height || (PAD_T + sorted.length * (barHeight + gap));
  const total   = sorted.reduce((s, r) => s + r.value, 0) || 1;
  const maxV    = Math.max(...sorted.map(r => r.value));
  ...
  const bars = sorted.map((row, i) => {
    const y    = PAD_T + i * (barHeight + gap);
    const cy   = y + barHeight / 2 + 4; // text baseline
    const barW = Math.max(2, _scale(row.value, 0, maxV, 0, trackW));

    running += row.value;
    const cumPct = running / total;
    cumPoints.push(`${(PAD_L + cumPct * trackW).toFixed(1)},${(y + barHeight / 2).toFixed(1)}`);

    const valStr = row.value >= 1000 ? (row.value / 1000).toFixed(0) + 'k' : row.value.toFixed(1);

    return `
    <text x="${PAD_L - 4}" y="${cy}" text-anchor="end" font-size="10" fill="${SLATE[600]}" dominant-baseline="middle">${row.label.slice(0, 14)}</text>
    <rect x="${PAD_L}" y="${y}" width="${barW.toFixed(1)}" height="${barHeight}" rx="3" fill="${ACCENT.value}" opacity="0.85"/>
    <text x="${PAD_L + trackW + 4}" y="${cy}" font-size="10" fill="${SLATE[600]}" dominant-baseline="middle">${valStr}</text>`;
  }).join('');

  const cumLine = `<polyline points="${cumPoints.join(' ')}" fill="none" stroke="${RAG_COLORS.red}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>`;
  const cumDots = cumPoints
    .map(p => {
      const [x, y] = p.split(',');
      return `<circle cx="${x}" cy="${y}" r="2" fill="${RAG_COLORS.red}"/>`;
    })
    .join('');
